      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 py-20 relative z-20 -mt-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {deploymentVectors.map((vector, index) => (
            <div 
              key={index} 
              className="group p-8 bg-white border border-slate-200 rounded-2xl shadow-xl hover:shadow-2xl hover:border-[#E5981A]/30 transition-all duration-300 transform hover:-translate-y-1 flex flex-col justify-between min-h-[280px]"
            >
              ...
              <div className="mt-8 border-t border-slate-100 pt-4 text-[9px] font-mono text-slate-400 tracking-wider">
                TICKETING_GRID // SECTOR_0{index + 1}
              </div>
            </div>
          ))}
        </div>
