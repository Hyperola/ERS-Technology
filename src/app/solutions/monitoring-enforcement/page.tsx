// src/app/solutions/revenue-monitoring-enforcement/page.tsx
import Link from 'next/link';

export default function RevenueMonitoringEnforcementPage() {
  const complianceSteps = [
    {
      title: "Real-Time Field Leakage Audits",
      desc: "Instantly cross-checks transactions across dispersed local field operations against central collection targets to identify inconsistencies the moment they occur."
    },
    {
      title: "Automated Compliance Gateways",
      desc: "Systematically flags unverified operator profiles and freezes suspicious transactional pipelines until structural validation requirements are met."
    },
    {
      title: "Geospatial Enforcement Tracking",
      desc: "Provides executive dashboards with detailed regional heatmaps tracking active physical collection nodes alongside verified digital compliance logs."
    }
  ];
    ...
    <main className="min-h-screen bg-[#F8FAFC] text-[#0A2540] overflow-x-hidden selection:bg-[#E5981A]/30 selection:text-[#0A2540] font-['Calibri',Calibri,Segoe_UI,sans-serif]">
      ...
      <section className="relative bg-[#0A2540] text-white py-24 px-6 md:px-12 lg:px-24 flex flex-col justify-center min-h-[50vh] overflow-hidden text-left">
        ...
        <div 
          ...
          style={{ backgroundImage: `url('https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?q=80&w=2000')` }}
        />
        ...
        <div className="max-w-5xl space-y-6 relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight leading-[1.1]">
            ...
            <span className="text-[#E5981A]">& Enforcement Systems</span>
          </h1>
          <p className="text-slate-200 text-lg md:text-xl max-w-3xl leading-relaxed font-normal">
            High-fidelity visibility frameworks and automated validation pipelines built to eliminate collections leakage, optimize compliance parameters, and protect public fiscal networks.
          </p>
        </div>
      </section>

      {/* 2. CORE ENFORCEMENT PRESENTATION GRID OVERLAY */}
      <section className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 py-20 relative z-20 -mt-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          
          {/* Left Summary Card */}
          <div className="lg:col-span-4 bg-white border border-slate-200 p-8 rounded-2xl flex flex-col justify-between relative overflow-hidden group hover:border-[#E5981A]/40 transition-all duration-300 shadow-xl min-h-[340px] text-left">
            <div className="space-y-4">
              <div className="h-2 w-2 rounded-full bg-[#E5981A]" />
              <h3 className="text-xs font-mono font-bold text-[#E5981A] tracking-widest uppercase">
                Fiscal Accountability Controls
              </h3>
              <p className="text-slate-600 text-sm font-normal leading-relaxed">
                We introduce real-time auditing logic to municipal and agency collections processes, transforming vulnerable manual reporting chains into absolute, tamper-evident digital compliance streams.
              </p>
            </div>
            
            <div className="mt-8 border-t border-slate-100 pt-6">
              <Link href="/contact" className="text-xs font-mono font-bold uppercase text-[#0A2540] hover:text-[#A62626] tracking-wider flex items-center gap-1 transition-colors">
                Request System Framework Audit <span className="group-hover:translate-x-1 transition-transform duration-200">&rarr;</span>
              </Link>
            </div>
          </div>

          {/* Right Core Pillars Layout */}
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-1 gap-6 text-left">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 shadow-xl space-y-6">
              <span className="text-[10px] font-mono font-bold text-[#E5981A] uppercase tracking-widest block border-b border-slate-100 pb-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {complianceSteps.map((step, idx) => (
                  <div key={idx} className="space-y-2 group">
                    <div className="text-xs font-mono font-bold text-slate-300 group-hover:text-[#A62626] transition-colors">
                      0{idx + 1}
                    </div>
      ...
      <section className="py-12 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto w-full mb-12">
        <div className="p-8 md:p-16 bg-[#0A2540] text-white rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden group text-center space-y-6">
          ...
          <h3 className="text-3xl md:text-5xl font-bold text-white tracking-tight max-w-3xl mx-auto leading-tight">
            Ready to Maximize Revenue Accountability?
          </h3>
          <p className="text-slate-300 text-sm md:text-base leading-relaxed font-normal max-w-2xl mx-auto">
            Bring institutional clarity, structured operator field verification, and automated anomaly flagging directly to your operations. Partner with our engineering group to consult on your system requirements.
          </p>
