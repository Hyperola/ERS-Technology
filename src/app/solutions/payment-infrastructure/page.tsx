    ...
    <main className="min-h-screen bg-[#F8FAFC] text-[#0A2540] overflow-x-hidden animate-fadeIn selection:bg-[#E5981A]/30 selection:text-[#0A2540] font-['Calibri',Calibri,Segoe_UI,sans-serif]">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 py-20 relative z-20 -mt-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          {/* Side Banner Card */}
          <div className="lg:col-span-4 bg-white border border-slate-200 p-8 rounded-2xl flex flex-col justify-between relative overflow-hidden group hover:border-[#E5981A]/40 transition-all duration-300 shadow-xl min-h-[320px] text-left">
          ...
          <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6 text-left">
            {nodes.map((node, index) => (
              <div 
                key={index} 
                className="group p-6 bg-white border border-slate-200 rounded-xl flex items-center justify-between shadow-xl hover:shadow-2xl hover:border-[#E5981A]/40 transition-all duration-300 transform hover:-translate-y-0.5"
              >
                <span className="text-sm font-bold text-[#0A2540] group-hover:text-[#A62626] transition-colors max-w-[80%]">
                  {node}
                </span>
                <span className="text-slate-300 group-hover:text-[#0A2540] text-xs font-mono font-bold transition-colors">
                  0{index + 1}
                </span>
              </div>
            ))}
