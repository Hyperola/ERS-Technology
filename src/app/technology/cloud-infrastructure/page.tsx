// src/app/technology/cloud-infrastructure/page.tsx
import Link from 'next/link';

export default function CloudInfrastructurePage() {
  const architecturalPillars = [
    {
      title: "High-Availability Clusters",
      desc: "Deploys multi-region active-active cluster nodes across continental server zones to secure continuous operation even under peak local transaction traffic spikes."
    },
    {
      title: "Encrypted State Remittance",
      desc: "Utilizes isolated, secure hardware security modules (HSM) to authenticate and isolate fiscal clearing data fields before dispatching logs to public systems."
    },
    {
      title: "Elastic Edge Scaling",
      desc: "Dynamically allocates local load-balancer resources to prioritize critical mobile terminal query volumes coming from distributed regional networks."
    }
  ];

  return (
    <main className="min-h-screen bg-[#F8FAFC] text-[#0A2540] overflow-x-hidden selection:bg-[#E5981A]/30 selection:text-[#0A2540] font-['Calibri',Calibri,Segoe_UI,sans-serif]">
      
      {/* 1. IMMERSIVE HERO BANNER MODULE */}
      <section className="relative bg-[#0A2540] text-white py-24 px-6 md:px-12 lg:px-24 flex flex-col justify-center min-h-[50vh] overflow-hidden text-left">
        <div 
          className="absolute inset-0 bg-cover bg-center mix-blend-luminosity opacity-15 scale-105"
          style={{ backgroundImage: `url('https://images.unsplash.com/photo-1600132806370-bf17e65e942f?q=80&w=2000')` }}
        />
        <div className="absolute inset-0 opacity-5 bg-[linear-gradient(to_right,#FFFFFF_1px,transparent_1px),linear-gradient(to_bottom,#FFFFFF_1px,transparent_1px)] bg-[size:4rem_4rem]" />
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-[#E5981A]/10 rounded-full blur-[120px] pointer-events-none" />

        <div className="max-w-5xl space-y-6 relative z-10">
          <div className="inline-flex items-center gap-2 border border-[#E5981A]/30 bg-white/5 backdrop-blur-sm px-4 py-2 rounded-full text-xs font-bold tracking-widest text-[#E5981A] uppercase">
            <span className="h-1.5 w-1.5 rounded-full bg-[#E5981A] animate-pulse" /> CLOUD PLATFORM SPECIFICATION
          </div>
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight leading-[1.1]">
            Cloud <br />
            <span className="text-[#E5981A]">Infrastructure</span>
          </h1>
          <p className="text-slate-200 text-lg md:text-xl max-w-3xl leading-relaxed font-normal">
            Secure and scalable cloud-based deployment architecture.
          </p>
        </div>
      </section>

      {/* 2. CORE ENFORCEMENT PRESENTATION GRID OVERLAY */}
      <section className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 py-20 relative z-20 -mt-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          
          <div className="lg:col-span-4 bg-white border border-slate-200 p-8 rounded-2xl flex flex-col justify-between relative overflow-hidden group hover:border-[#E5981A]/40 transition-all duration-300 shadow-xl min-h-[340px] text-left">
            <div className="space-y-4">
              <div className="h-2 w-2 rounded-full bg-[#E5981A]" />
              <h3 className="text-xs font-mono font-bold text-[#E5981A] tracking-widest uppercase">
                Enterprise Scalability Controls
              </h3>
              <p className="text-slate-600 text-sm font-normal leading-relaxed">
                We engineer decentralized, high-throughput cloud spaces that provide automated resource distribution across municipal clusters, matching strict international financial data isolation metrics.
              </p>
            </div>
            
            <div className="mt-8 border-t border-slate-100 pt-6">
              <Link href="/contact" className="text-xs font-mono font-bold uppercase text-[#0A2540] hover:text-[#A62626] tracking-wider flex items-center gap-1 transition-colors">
                Request Deployment Specs <span className="group-hover:translate-x-1 transition-transform duration-200">&rarr;</span>
              </Link>
            </div>
          </div>

          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-1 gap-6 text-left">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 shadow-xl space-y-6">
              <span className="text-[10px] font-mono font-bold text-[#E5981A] uppercase tracking-widest block border-b border-slate-100 pb-3">
                Cloud Architecture Framework
              </span>
              
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {architecturalPillars.map((step, idx) => (
                  <div key={idx} className="space-y-2 group">
                    <div className="h-1 w-4 bg-slate-200 group-hover:bg-[#A62626] transition-colors mb-2" />
                    <h4 className="font-bold text-[#0A2540] text-base">
                      {step.title}
                    </h4>
                    <p className="text-slate-500 text-xs leading-relaxed font-normal">
                      {step.desc}
                    </p>
                  </div>
                ))}
              </div>
            </div>
          </div>

        </div>
      </section>

      {/* 3. HIGH-TECH INSTITUTIONAL ACTION PANEL */}
      <section className="py-12 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto w-full mb-12">
        <div className="p-8 md:p-16 bg-[#0A2540] text-white rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden group text-center space-y-6">
          <div className="absolute -right-20 -bottom-20 w-64 h-64 rounded-full border border-white/5 group-hover:scale-110 transition-transform duration-700 pointer-events-none" />
          
          <span className="inline-block text-[9px] font-mono tracking-widest bg-white/5 border border-white/10 px-4 py-1 rounded-full text-[#E5981A] font-bold uppercase">
            Platform Infrastructure Layer
          </span>
          <h3 className="text-3xl md:text-5xl font-bold text-white tracking-tight max-w-3xl mx-auto leading-tight">
            Ready to Deploy Scalable Cloud Architecture?
          </h3>
          <p className="text-slate-300 text-sm md:text-base leading-relaxed font-normal max-w-2xl mx-auto">
            Bring high availability, absolute geographic server protection, and zero-downtime ledger accounting to your municipal deployment. Partner with our engineering group to customize your parameters.
          </p>
          <div className="pt-4 flex flex-col sm:flex-row gap-4 w-full justify-center max-w-md mx-auto">
            <Link 
              href="/contact" 
              className="bg-[#A62626] hover:bg-[#A62626]/90 text-white text-center font-bold tracking-wider text-xs uppercase px-8 py-4 rounded-xl transition-all shadow-md w-full active:scale-95 duration-150"
            >
              Consult with Infrastructure Engineers
            </Link>
          </div>
        </div>
      </section>

    </main>
  );
}